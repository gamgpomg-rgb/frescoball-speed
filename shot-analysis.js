...
function geometry(p,k){
 ...
 return {lateral:Math.abs((p[k].x-sx)*(-dy/h)+(p[k].y-sy)*(dx/h))/w,height:((p[k].x-hx)*dx+(p[k].y-hy)*dy)/(h*h),x:(p[k].x-sx)/h,y:(p[k].y-sy)/h};
}
function classify(events,frames){
 return (events||[]).filter(e=>['auto','confirmed'].includes(e.status)&&['a','b'].includes(e.player)).map(e=>{
  ...
  const type=lateral>=.72?'attack':lateral<=.52?'defense':null;
...
function summary(items){const attack=items.filter(e=>e.type==='attack').length,defense=items.filter(e=>e.type==='defense').length;return {attack,defense,classified:attack+defense,total:items.length};}
return {classify,summary,inferAlternation};
});
