...
find('1人目を選ぶ').onclick();tap(30,30);tap(220,440);assert(R.snapshot().events.every(e=>e.status==='pending'),'changing target invalidates confirmations');
...
(async()=>{
  ...
  await pick('選んだ2人で自動解析').onclick();assert.equal(audioCalls,0,'audio must wait for selecting both people');
