...
(async()=>{
  ...
  c.curStats.audioHits=Array.from({length:30},(_,i)=>({t:1+i*.5,player:i%2?'b':'a'}));
  const sparse={settings:{distanceM:7},frames:[{t:1}],events:c.curStats.audioHits.map((h,i)=>({...h,status:i===0?'auto':'pending',player:i===0?'a':null,origin:'audio'}))};
  workflow.onChange(sparse);assert.equal(c.curStats.total,30,'additional analysis preserves every audio hit');assert.equal(c.curStats.rallies,1,'missing poses never split the rally');
